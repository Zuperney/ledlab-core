...
describe("cadeia de verificação no Caderno (rigCadeia)", () => {
  ...
  it("a cadeia NÃO dimensiona: sem elo de talha nem de barra (isso é do rigger)", () => {
    const c = rigCadeia(tela(comLimite));
    expect(c.find((e) => e.id === "talha")).toBeUndefined();
    expect(c.find((e) => e.id === "porBarra")).toBeUndefined();
  });

  it("a cadeia termina no que o app NÃO alcança", () => {
    const c = rigCadeia(tela(comLimite));
    const fim = c[c.length - 1];
    expect(fim.titulo).toBe("Bumpers, talhas, treliça e piso");
...
describe("status e textos da seção", () => {
  ...
  it("o texto do estouro desarma a saída errada (mais ferragem)", () => {
    ...
    expect(txt).toContain("ferragem não resolve");
    expect(txt).toContain("3 m contra 2 m publicados"); // nomeia o número que estourou
  });
